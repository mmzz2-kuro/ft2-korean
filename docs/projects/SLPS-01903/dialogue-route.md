...
### 2026-07-09 추가 재확인: 범위가 더 넓다, `0xC4CC000`도 진짜 끝이 아니었다

위 범위(`0xC024000..0xC4CC000`)만 스캔한 결과가 89행이었는데, 유저가
"아직 누락 대사가 다 안 들어왔다"고 지적해서 ink 하한을 낮추고
(`100`→`15`) 양쪽으로 범위를 더 넓혀(`0xBEF0000..0xC540000`) 재스캔한
결과 28개 후보가 추가로 나왔다(그중 27개가 실제 대사로 확인, 최종
112행). 확인된 것:

1. **`0xBF00000..0xBFEC000` 구간에 완전히 새로운 블록이 있었다** — 알의
   정체에 대한 핵심 설정 노출부다: "彼は、無力な人間の姿となり、再び
   甦るための眠りについた。しかし魂がなければ肉体は死んでしまう。
   そこで、アル・・君という人格が生まれたのだ" (그는 무력한 인간의
   모습이 되어, 다시 되살아나기 위한 잠에 들었다. 하지만 혼이 없으면
   육체는 죽어버린다. 그래서, 알... 이라는 인격이 태어난 것이다) 등.
   즉 `0xC024000`이 이 영역의 시작이 아니었다.
2. 이미 스캔한 범위 안에서도 **ink가 낮은(짧은) 리액션 대사**가 여러 개
   빠져 있었다(`アル!?`, `グッ!?`, `う・・・ん・・・` 등). ink 하한
   `100`은 너무 높았다.
3. **`0xC4CC000`("そこが、僕たちの帰る場所です")도 진짜 마지막이
   아니었다** — 그 뒤로 기상/일상 대화로 보이는 짧은 에필로그가 더
   있다(`0xC4DA000`, `0xC4EE000`, `0xC502000`, `0xC52B000`: "なぜなら、
   今や君は、アルと言う一人の人間なのだから・・・" → "やあ、眠れた
   かい?" → "うん・・・ちょっとだけ。サーラは?" → "さっき、出て
   行ったよ。声をかけようとしたんだが・・・"). 유저가 확인한 "마지막
   대사"는 아마 엔딩 컷신의 마지막 줄이었을 뿐, 그 뒤에도 짧은 코다
   신이 더 있는 것으로 보인다.

**새로 알아낸 false-positive 패턴**: `0xC479800`은 ink/segment count
필터를 둘 다 통과했지만 실제로는 새 메시지가 아니라, 0x800바이트
뒤에 있는 진짜 메시지(`0xC47A000`, "僕には、もう何の力もありません")의
데이터를 일부 겹쳐 읽은 것이었다(0x930바이트 창이 앞쪽 노이즈 +
다음 메시지 시작부 조각을 같이 담음). 실제 메시지 바로 앞 섹터에서
이런 "겹쳐읽기" 후보가 드물게 나올 수 있으니, 스캔 결과에서 바로
다음 후보와 매우 가깝고(간격이 비정상적으로 좁음) 내용이 다음 항목의
꼬리처럼 보이면 삭제 대상이다.

**결론**: 이 영역은 원래 추정보다 넓다(`0xBF00000`부터 최소
`0xC52B000`+ 까지, 5.7MB+). 지금까지도 "끝"이라고 확인했던 지점이
두 번(`16108`, `0xC4CC000`) 다 실제로는 끝이 아니었으므로, 현재
`finale-text.tsv` 112행도 아직 전체가 아닐 가능성이 높다. 스캔 범위를
이미 확인된 시작/끝보다 여유 있게(최소 1MB) 넓게 잡고, ink 하한도
낮게(`15`~`20`) 잡아서 재스캔하는 것을 권장한다.

### 2026-07-09 추가: 마을 순회 구간 발견 + 이 포맷이 게임 전체에서 쓰인다는 것 확인

유저가 윈도우판 EVENT17 참고 데이터(`church`/`weapon`/`weapon_buy`/
`store`/`macdogal`/`guild`/`departure` 이벤트, offset 약 4143..9026)에
대응하는 PS1 대사가 아직 안 잡혔다고 지적했다. 이건 최종 던전 가기
전에 마을을 돌며 작별 인사를 나누는 구간이다.

`0xB000000..0xC600000`로 스캔 범위를 넓혀 확인한 결과, **`0xB000000
..0xBEEE000`(약 15MB)에 이 마을 순회 구간 대사가 통째로 있었다**
(예: "시간의 신전", 카린이 과거를 바꾸려는 시도, 길드/교회 작별
인사, "早く出発しよーぜ!?"). 32개 샘플 전부 실제 대사로 확인.

**더 큰 발견**: 이 범위(`0xB000000..0xC600000`) 밖에서도 스캔 필터
(ink+segment count)를 통과하는 후보가 2418개 나왔는데, 무작위 10개를
확인해보니 7개가 실제 대사였다 — 그런데 17일차와 무관한, 게임 초반/
중반의 완전히 다른 날 대사들이었다(예: "母さん・・・私はどうすれば
いいんだ・・・", "これはマジでやばいってんで、ギルドが本格的に
調査することになった"). 즉 **이 `segment count 1..52` 블록 포맷은
17일차 전용이 아니라 게임 전체 대사 저장에 쓰이는 범용 포맷**으로
보인다 — 아마 지금까지 찾은 그룹 0~15 메시지뱅크 시스템과는 별개로,
전체 게임 스크립트가 곳곳에 이 포맷으로 흩어져 있을 가능성이 있다.

**범위 결정**: 지금은 17일차 범위(`0xB000000..0xC600000`, 확인된
359개)만 `finale-text.tsv`에 유지하고 번역 작업을 진행한다. 나머지
2418개(노이즈 섞여 있음, 대략 70~80% 실제 대사로 추정)는
`tmp/SLPS-01903/finale-text-workflow/whole-game-extra-dialogue.tsv`에
그대로 보관해뒀다 — "게임 전체 대사 재발굴"은 별도 과제로 나중에
다룬다.
