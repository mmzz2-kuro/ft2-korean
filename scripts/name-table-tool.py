...
# -*- coding: utf-8 -*-
"""
SLPS-01903 리소스 829 고유명사(캐릭터/몬스터/직업/NPC/상태이상/아이템/마법/장비창 캐릭터명/장비 스탯 라벨) 테이블 export/patch 도구.

포맷 (라이브 디버깅으로 확인됨), 리소스 829 payload 안에 서로 다른 일곱 개의
1bpp 이름 비트맵 테이블이 있다:
  ...
  일곱 테이블 모두: 바이트 MSB부터 순서대로 1픽셀씩, bit=0 -> 잉크(글자),
